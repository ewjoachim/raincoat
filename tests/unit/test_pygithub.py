...
@pytest.fixture
def pygithub_match():
    return pygithub.PyGithubMatch(repo="python/cpython@abc123",
                                  path="Lib/this.py", element="s",
                                  branch="3.6",
                                  filename="filename", lineno=12)


def test_match_str(pygithub_match):
    assert(
        str(pygithub_match) ==
        "python/cpython@abc123 vs 3.6 branch at Lib/this.py:s "
        "(from filename:12)")
...
def test_wrong_package_format():
    with pytest.raises(pygithub.NotMatching):
        pygithub.PyGithubMatch(filename="a", lineno=12, repo="bla",
                               path="path")


def test_current_source_key(mocker, pygithub_match):
    mocker.patch(
        "raincoat.source.get_branch_commit",
        return_value="aaabbbcccdddeeefff")

    assert pygithub.PyGithubChecker().current_source_key(pygithub_match) == (
        "python/cpython", "aaabbbcccdddeeefff")
...
def test_get_source_installed(mocker):
    ...
    result = pygithub.PyGithubChecker().get_source(
        key=pygithub.PyGithubKey("python/cpython", "123abc"),
        files=["file_1.py"])

    assert result == {"file_1.py": ["yay"]}
    assert download.mock_calls == [
        mocker.call(repo="python/cpython", commit="123abc",
                    files=["file_1.py"])]
